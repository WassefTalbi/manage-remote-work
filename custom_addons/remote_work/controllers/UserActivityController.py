...
class UserActivityController(http.Controller):
    ...
    @http.route("/api/employee_daily_work", methods=["GET"], type="http", auth="none", csrf=False)
    def log_employee_daily_work(self, **post):
        try:

            auth_header = http.request.httprequest.headers.get('Authorization')
            if not auth_header or not auth_header.startswith('Bearer '):
                raise AccessDenied("Missing token")
            ...
            if not current_attendance:
                ...
                raise ValueError("No active attendance record found")
            ...
            if data.get('app_usage'):
                ...
                for app in data['app_usage']:
                    ...
                    if isinstance(app, dict):
                        ...
                        apps_to_create.append({
                            'name': app.get('name', 'Unknown'),
                            'formatted_time': app.get('time_spent'),
                            'activity_id': activity.id
                        })

                    else:
                        _logger.warning(f"Invalid app data: {app}")


                if apps_to_create:
                    http.request.env['app.usage.entry'].sudo().create(apps_to_create)


            if data.get('site_usage'):
                sites_to_create = []
                for site in data['site_usage']:
                    if isinstance(site, dict):

                        sites_to_create.append({
                            'name': site.get('name', 'Unknown'),
                            'formatted_time': site.get('time_spent'),
                            'activity_id': activity.id
                        })

                    else:
                        _logger.warning(f"Invalid site data: {site}")


                if sites_to_create:
                    http.request.env['site.usage.entry'].sudo().create(sites_to_create)


            return http.Response(json.dumps({'status': 'success'}), mimetype='application/json')

        except Exception as e:
            _logger.exception("Failed to process employee_daily_work")
            return http.Response(json.dumps({'error': str(e)}), status=500, mimetype='application/json')
